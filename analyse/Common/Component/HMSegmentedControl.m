...
#ifndef _Common_HMSegmentedControl_
#define _Common_HMSegmentedControl_
...
- (void)commonInit {
    ...
    self.fixedIndicatorSize = CGSizeMake(16, 3);
    self.fixedIndicatorTopOffset = 1;
    self.fixedIndicatorBottomOffset = 8;
...
//            CGFloat y = roundf((CGRectGetHeight(self.frame) - selectionStyleNotBox * self.selectionIndicatorHeight) / 2 - stringHeight / 2 + self.selectionIndicatorHeight * locationUp);
            CGFloat y = self.bounds.size.height - self.fixedIndicatorBottomOffset - self.fixedIndicatorTopOffset - self.fixedIndicatorSize.height - stringHeight;
    ...
    if (self.selectedSegmentIndex != HMSegmentedControlNoSegment) {
        if (self.selectionStyle == HMSegmentedControlSelectionStyleArrow) {
            ...
            if (!self.selectionIndicatorStripLayer.superlayer) {
                ...
                if (self.selectionIndicatorColor == [UIColor clearColor]) {
                    self.gradientLayer.frame = [self frameForSelectionIndicator];
...
- (CGRect)frameForSelectionIndicator {
    ...
    if (self.selectionStyle == HMSegmentedControlSelectionStyleTextWidthStripe &&
        ...
        CGFloat x = widthToStartOfSelectedIndex + (self.segmentWidth - self.fixedIndicatorSize.width) / 2;
        CGFloat y = self.bounds.size.height - self.fixedIndicatorSize.height - self.fixedIndicatorBottomOffset;
        return CGRectMake(x, y, self.fixedIndicatorSize.width, self.fixedIndicatorSize.height);
    } else {
        if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleDynamic) {
            CGFloat selectedSegmentOffset = 0.0f;
            ...
            CGFloat x = selectedSegmentOffset + ([[self.segmentWidthsArray objectAtIndex:self.selectedSegmentIndex] floatValue] - self.fixedIndicatorSize.width) / 2;
            CGFloat y = self.bounds.size.height - self.fixedIndicatorSize.height - self.fixedIndicatorBottomOffset;
            return CGRectMake(x, y, self.fixedIndicatorSize.width, self.fixedIndicatorSize.height);
